            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        datType: 'json',
                        data: {
                            product_plan_id: '{{ \Illuminate\Support\Facades\Crypt::encrypt($product_plan->id) }}',
                            coupon: coupon
                        },
            ...
            function payWithPaystack(amount) {
                ...
                var handler = PaystackPop.setup({
                    ...
                    callback: function(response) {
                        {{-- console.log(paystack_callback +'/'+ response.reference + '/' + '{{\Illuminate\Support\Facades\Crypt::encrypt($product_plan->id)}}'); --}}
                        window.location.href = paystack_callback + '/' + response.reference + '/' +
                            '{{ \Illuminate\Support\Facades\Crypt::encrypt($product_plan->id) }}?coupon_id=' +
                            coupon_id;
                    },
                    onClose: function() {
                        alert('window closed');
                        ...
                        <div class="mt-5">
                            <div class="card price-card price-1 wow animate__fadeInUp" data-wow-delay="0.2s"
                                ...
                                <div class="card-body">
                                    ...
                                    @if (\Auth::user()->type == 'Owner' && \Auth::user()->plan == $product_plan->id)
                                        ...
                                        <div class="mb-3">
                                            @if (\Auth::user()->type == 'super admin')
                                                <div class="action-btn bg-primary ms-2">
                                                    <a href="#"
                                                        data-url="{{ route('product-plan.edit', $product_plan->id) }}"
                                        ...
                                        @if ($product_plan->price)
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('Enable Products') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Enable Products') }}
                                            </li>
                                        @endif
                                    </ul>
                        ...
                        <div id="stripe_payment" class="card">
                            ...
                            @if (
                                ...
                                <div class="tab-pane {{ ($admin_payments_details['is_stripe_enabled'] == 'on' && !empty($admin_payments_details['stripe_key']) && !empty($admin_payments_details['stripe_secret'])) == 'on' ? 'active' : '' }}"
                                    id="stripe_payment">
                                    <form role="form" action="{{ route('stripe.payv4') }}" method="post"
                                        ...
                                        <div class="col-sm-12 my-2 px-2">
                                            <div class="text-end">
                                                ...
                                                <input type="hidden" name="product_plan_id"
                                                    value="{{ \Illuminate\Support\Facades\Crypt::encrypt($product_plan->id) }}">
                            ...
                            @if (
                                ...
                                <div class="tab-pane {{ ($admin_payments_details['is_stripe_enabled'] != 'on' && $admin_payments_details['is_paypal_enabled'] == 'on' && !empty($admin_payments_details['paypal_client_id']) && !empty($admin_payments_details['paypal_secret_key'])) == 'on' ? 'active' : '' }}"
                                    ...
                                    <form class="w3-container w3-display-middle w3-card-4" method="POST"
                                        id="payment-form" action="{{ route('product.plan.pay.with.paypal') }}">
                                        @csrf
                                ...
                                <div class="tab-pane " id="paytm_payment">
                                    <form class="w3-container w3-display-middle w3-card-4" method="POST"
                                        ...
                                        <input type="hidden" name="total_price" id="paytm_total_price"
                                            value="{{ $product_plan->price }}" class="form-control">
                                        <div class="border p-3 mb-3 rounded payment-box">
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="form-group">
