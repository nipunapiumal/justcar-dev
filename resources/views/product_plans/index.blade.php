...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ __('Product Plans') }}</li>
@endsection
@section('title')
    <div class="d-inline-block">
        <h5 class="h4 d-inline-block text-white font-weight-bold mb-0 ">{{ __('Product Plans') }}</h5>
    </div>
@endsection
@section('action-btn')
        ...
        @if (
            ...
            <div class="pr-2">
                <a href="#" data-ajax-popup="true" data-size="lg" data-title="{{ __('Add Plan') }}"
                    data-url="{{ route('product-plan.create') }}" class="btn btn-sm btn-primary btn-icon m-1"
            ...
            <div class="plan_card">
                <div class="card price-card price-1 wow animate__fadeInUp" data-wow-delay="0.2s"
                    style="
                                  ...
                                  ">
                    <div class="card-body">
                        <span class="price-badge bg-primary">{{ $product_plan->name }}</span>
                        @if (\Auth::user()->type == 'Owner' && \Auth::user()->product_plan == $product_plan->id)
                            ...
                            <div class="mb-3">
                                @if (\Auth::user()->type == 'super admin')
                                    <div class="action-btn bg-primary ms-2">
                                        <a href="#" data-url="{{ route('product-plan.edit', $product_plan->id) }}"
                        ...
                        <h4 class="f-w-600">
                            {{ $product_plan->price }}{{ env('CURRENCY_SYMBOL') }}
                        </h4>
                        <h6 class="f-w-600">
                            ...
                            @if ($product_plan->price)
                                <li>
                                    <span class="theme-avtar">
                                        <i class="text-primary fas fa-check"></i></span>{{ __('Enable Products') }}
                                </li>
                            @else
                                <li class="text-danger">
                                    <span class="theme-avtar">
                                        <i class="text-danger fas fa-times-circle"></i></span>{{ __('Enable Products') }}
                                </li>
                            @endif
                        </ul>
                        ...
                        <div class="row">
                            @if (\Auth::user()->type != 'super admin')
                                @if (\Auth::user()->product_plan == $product_plan->id && date('Y-m-d') < \Auth::user()->product_plan_expire_date)
                                    <h5 class="h6 my-4">
                                        {{ __('Expired : ') }}
                                        {{ \Auth::user()->product_plan_expire_date ? \App\Models\Utility::dateFormat(\Auth::user()->product_plan_expire_date) : __('Unlimited') }}
                                    </h5>
                                @elseif(
                                ...
                                @elseif(\Auth::user()->product_plan == $product_plan->id)
                                    <div class="col-12">
                                ...
                                @else
                                    <div class="col-12">
                                        <a href="{{ route('stripe', ['code' => \Illuminate\Support\Facades\Crypt::encrypt($product_plan->id), 'type' => 'product']) }}"
