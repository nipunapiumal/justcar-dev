...
<head>
    @include('storefront.layout.theme29to34.header-type-1')
</head>
...
<body class="tt-magic-cursor">
                    ...
                    <li>
                        <a href="{{ route('store.wishlist', $store->slug) }}">
                            ...
                            <span class="wishlist_count text-uppercase"
                                id="wishlist_count">({{ !empty($wishlist) ? count($wishlist) : '0' }})
                                {{ __('Save') }}</span>
                        </a>
                    </li>
                @endif
    ...
    <header class="header-area style-1">
        <div class="header-logo d-lg-none d-flex">
            <a class="navbar-brand logos" href="{{ route('store.slug', $store->slug) }}">
                @if (!empty($store->logo_dark))
                    <img class="img-fluid" src="{{ asset(Storage::url('uploads/store_logo/' . $store->logo_dark)) }}"
                        alt="Image placeholder">
                @else
                    <img class="img-fluid" src="{{ asset(Storage::url('uploads/store_logo/logo.png')) }}"
                        alt="Image placeholder">
                @endif
            </a>
        </div>
        <div class="main-menu">
            ...
            <ul class="menu-list">
                {{-- <li>
                    <a class="drop-down" href="{{ route('store.slug', $store->slug) }}">
                        {{ __('Home') }}
                    </a>
                </li> --}}
                @php
                ...
                @if ($storethemesetting['primary_nav_menu'])
                    @foreach (json_decode($storethemesetting['primary_nav_menu']) as $menu_title)
                        @if ($info = Utility::generateLink($menu_title, $storethemesetting, $store, $plan))
                            <li>
                                <a class="drop-down" href="{{ $info['link_url'] }}">
                                    {{ $info['link_name'] }}
                                </a>
                            </li>
                        @endif
            ...
            <div class="topbar-right d-lg-none d-block">
                ...
                @if (Utility::CustomerAuthCheck($store->slug) == true)
                    <a href="{{ route('store.wishlist', $store->slug) }}">
                        ...
                        {{ __('Save') }} <span class="wishlist_count text-uppercase"
                            id="wishlist_count">({{ !empty($wishlist) ? count($wishlist) : '0' }})
                        </span>
                    </a>
                @endif
        ...
        <div class="nav-right d-flex jsutify-content-end align-items-center">
            ...
            <div class="hotline-area d-xl-flex d-none">
                <div class="icon">
                    <img src="{{ asset('assets/theme29to34/img/home1/icon/hotline-icon.svg') }}" alt="">
                </div>
                <div class="content">
                    <span>{{ __('Phone Support') }}</span>
                    <h6>
    ...
    @include('storefront.layout.theme29to34.footer-type-1')
</body>
