        ...
        <div class="slider-wrapper">
            @if (isset($storethemesetting['enable_slider_settings']) && $storethemesetting['enable_slider_settings'] == 'on')
                @foreach (json_decode($storethemesetting['slider_settings']) as $key => $sliderSettings)
                    <div data-dot="0{{ $key }}" class="single-slide"
                ...
                @endforeach
            @else
                <div data-dot="0" class="single-slide"
                                    ...
                                    <div class="feature-image">
                                        <a href="{{ route('store.product.product_view', [$store->slug, $product->id]) }}">
                                            @if (!empty($product->is_cover) && \Storage::exists('uploads/is_cover_image/' . $product->is_cover))
                                                <img alt="Image placeholder" width="371" height="493"
                                ...
                                @if ($product_count[$key] > 0)
                                    <div class="p-lr-15px">
                                        <div class="single-blog">
                                            ...
                                            <div class="blog-text">
                                                <h5><a href="{{ route('store.categorie.product', [$store->slug, $pro_categorie->name]) }}">{{ $pro_categorie->name }}</a>
                                                </h5>
                                                <p>
                                                    {{ __('Products') }}:
                                                    {{ !empty($product_count[$key]) ? $product_count[$key] : '0' }}
                                                </p>
                                                <a href="{{ route('store.categorie.product', [$store->slug, $pro_categorie->name]) }}" class="default-btn"><span>{{__('Show More')}}</span></a>
                                            </div>
                                        </div>
                                    </div>
                                @endif
