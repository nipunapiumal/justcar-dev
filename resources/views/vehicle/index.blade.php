...
@section('title')
    <div class="d-inline-block">
        <h5 class="h4 d-inline-block text-white font-weight-bold mb-2">{{ __('Vehicle') }}</h5>
    </div>
@endsection
@section('breadcrumb')
                                        ...
                                        <td class="Action">
                                            <span>
                                                ...
                                                <div class="action-btn  bg-info ms-2">
                                                    <a href="{{ $product->product_type == 1 ? route('product.edit', $product->id) : route('vehicle.edit', $product->id) }}"
                                                        class="mx-3 btn btn-sm d-inline-flex align-items-center"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="{{ __('Edit') }}"><i
                                                ...
                                                <div class="action-btn bg-info ms-2" style="position: relative;top:-1px">
                                                    <a href="{{ route('products.view.pdf', $product->id) }}"
                                                        class="mx-3 btn btn-sm d-inline-flex align-items-center"
                                                        data-toggle="tooltip" data-original-title="{{ __('Generate Sales Poster') }}"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="{{ __('Generate Sales Poster') }}"
                                                        data-tooltip="{{ __('Generate Sales Poster') }}"><i
                                                            class="fas fa-qrcode text-white"></i></a>
                                                </div>
                                                <div class="action-btn bg-info ms-2" style="position: relative;top:-1px">
                                                    <a href="#" data-size="lg" data-url="{{ route('invoice.create', ['id'=>$product->id])  }}" data-ajax-popup="true"
                                                        data-title="{{ __('Generate Invoice') }}" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Generate Invoice ') }}"
                                                        class="mx-3 btn btn-sm d-inline-flex align-items-center">
                                                        <i class="far fa-file-alt text-white"></i>
                                                    </a>
                                                </div>
                                                @endif
                                                @if ($store_id->is_mobilede_api_enabled == 'on' && $product->product_type == 2)
                                                    <div class="action-btn bg-info ms-2"
                                                        style="position: relative;top:-1px">

                                                        @if ($product->mobilede_ad_id)
                                                            <a href=""
                                                                class="mx-3 btn btn-sm d-inline-flex align-items-center disabled"><i
                                                                    class="fas fa-upload text-white"></i></a>
                                                        @else
                                                            <a href="{{ route('product.mobile.de.send', $product->id) }}"
                                                                class="mx-3 btn btn-sm d-inline-flex align-items-center"
                                                                data-toggle="tooltip"
                                                                data-original-title="{{ __('Mobile.de upload') }}"
                                                                data-bs-toggle="tooltip" data-bs-placement="top"
                                                                title="{{ __('Mobile.de upload') }}"
                                                                data-tooltip="{{ __('Mobile.de upload') }}"><i
                                                                    class="fas fa-upload text-white"></i></a>
                                                        @endif
                                                    </div>
                                                @endif
