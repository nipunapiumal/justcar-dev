                    ...
                    <div class="col-sm-5">
                        <div class="card min-h-390">
                            ...
                            <div class="card-header d-flex justify-content-between">
                                <h5>{{ __('Available Storage') }}</h5>
                            </div>

                            <div class="card-body text-center">
                                <div id="av_storage"></div>
                                <h6 class="mb-3">{{ __('Available Storage') }} ({!! $storage_percentage !!}%)<h6>
                                        <a href="{{ route('storage_plans.index') }}"
                                            class="btn btn-primary btn-sm text-sm">
                                            {{ __('Buy More') }} {{ __('Storage') }}
                                        </a>
                            </div>
                        </div>
                    </div>
                                                            ...
                                                            <td>
                                                                <div class="d-flex align-items-center">
                                                                    <a href="{{ route('orders.show', \Illuminate\Support\Facades\Crypt::encrypt($order->id)) }}"
                                                                        ...
                                                                        title="{{ __('Details') }}" data-toggle="tooltip"
...
@push('script-page')
    ...
    @if (\Auth::user()->type == 'Owner')
        @php $setting = \App\Models\Setting::where('name', 'term_and_condition')->first(); @endphp
        @if (\Auth::user()->term_accepted_at < $setting->updated_at)
            <script>
                var url = '{{ route('users.accept-terms') }}'
    ...
    @if (\Auth::user()->type == 'Owner')
        <script>
            var options = {
                ...
                labels: ['{!! $available_storage !!}MB / {!! $total_storage !!}MB'],
            };
