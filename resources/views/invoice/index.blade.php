...
@section('title')
    <div class="d-inline-block">
        <h5 class="h4 d-inline-block text-white font-weight-bold mb-0">{{ __('Invoice') }}</h5>
    </div>
@endsection
@section('breadcrumb')
...
@section('action-btn')
    {{-- <div class="pr-2">
        <a href="#" data-size="md" data-url="{{ (Request::route()->getName()=='vehicle_tax.index'?route('vehicle_tax.create'):route('product_tax.create'))  }}" data-ajax-popup="true"
            data-title="{{ __('Create New Tax') }}" class="btn btn-sm btn-primary btn-icon m-1"
                ...
                <div class="card-body table-border-style">
                    ...
                    <div class="table-responsive">
                        <table class="table mb-0 dataTable ">
                            <thead>
                                <tr>
                                    <th scope="col" class="sort" data-sort="invoice_no">{{ __('Invoice Number') }}</th>
                                    <th scope="col" class="sort">{{ __('Vehicle Number') }}</th>
                                    <th scope="col" class="sort">{{ __('Customer Name') }}</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($invoices as $invoice)
                                    <tr data-name="{{ $invoice->invoice_no }}">
                                        <td>{{ $invoice->invoice_no }}</td>
                                        <td>{{ $invoice->vehicle->vehicle_number }}</td>
