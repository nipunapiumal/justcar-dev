...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ __('Storage Plans') }}</li>
@endsection
@section('title')
    <div class="d-inline-block">
        <h5 class="h4 d-inline-block text-white font-weight-bold mb-0 ">{{ __('Storage Plans') }}</h5>
    </div>
@endsection
@section('action-btn')
        ...
        @if (
            ...
            <div class="pr-2">
                <a href="#" data-ajax-popup="true" data-size="lg" data-title="{{ __('Add Plan') }}"
                    data-url="{{ route('storage_plans.create') }}" class="btn btn-sm btn-primary btn-icon m-1"
...
@section('content')
    <div class="row">
        @foreach ($storage_plans as $storage_plan)
            <div class="plan_card">
                <div class="card price-card price-1 wow animate__fadeInUp" data-wow-delay="0.2s"
                    style="
                                  ...
                                  ">
                    <div class="card-body">
                        <span class="price-badge bg-primary">{{ $storage_plan->name }}</span>
                        @if (\Auth::user()->type == 'Owner' && \Auth::user()->storage_plan == $storage_plan->id)
                            ...
                            @elseif(\Auth::user()->type == 'Owner' && Utility::getStorageUsage() >= $storage_plan->capacity)
                            <div class="d-flex flex-row-reverse m-0 p-0 plan-active-status">
                                <span class="d-flex align-items-center ">
                                    <i class="lh-1 far fa-question-circle text-danger" data-bs-toggle="tooltip"
                                    data-bs-placement="top"
                                    title="{{ __('Your storage usage is higher than this plan') }}"></i>
                                </span>
                            </div>
                        @endif
                            ...
                            <div class="mb-3">
                                @if (\Auth::user()->type == 'super admin')
                                    <div class="action-btn bg-primary ms-2">
                                        <a href="#" data-url="{{ route('storage_plans.edit', $storage_plan->id) }}"
                        ...
                        <h6 class="f-w-600">
                            @if ($storage_plan->price == 0)
                                {{ __('Duration') }} / {{ __('Unlimited') }}
                            @else
                                {{ __('Duration') }} / {{ env('STORAGE_PLAN_DURATION') }} {{ __('Months') }}
                            @endif
                        </h6>
                        @if (App\Models\StorageOrder::getCredit() && \Auth::user()->type == 'Owner' && \Auth::user()->storage_plan != $storage_plan->id && $storage_plan->price)
                            <h6 class="f-w-600">
                                {{ __('Credits') }}

                                / {{ App\Models\StorageOrder::getCredit() }}{{ env('CURRENCY_SYMBOL') }}
                                <small>
                        ...
                        <div class="row">
                            ...
                            @if (\Auth::user()->storage_plan == $storage_plan->id && date('Y-m-d') < \Auth::user()->storage_plan_expire_date)
                                    <h5 class="h6 my-4">
                                        {{ __('Expired : ') }}
                                        {{ \Auth::user()->storage_plan_expire_date ? \App\Models\Utility::dateFormat(\Auth::user()->storage_plan_expire_date) : __('Unlimited') }}
                                    </h5>
                                @elseif(
                                ...
                                @elseif(\Auth::user()->storage_plan != $storage_plan->id && Utility::getStorageUsage() >= $storage_plan->capacity)
                                    <div class="col-12">
                                ...
                                {{-- @elseif(\Auth::user()->storage_plan == $storage_plan->id)
                                    <div class="col-12">
                                        <a href="javascript:void(0)"
                                            class="btn btn-secondary disabled d-flex justify-content-center align-items-center ">{{ __('Subscribe') }}
                                            <i class="fas fa-arrow-right m-1"></i></a>
                                ...
                                @else
                                    <div class="col-12">
                                        <a href="{{ route('stripe', ['code' => \Illuminate\Support\Facades\Crypt::encrypt($storage_plan->id), 'type' => 'storage']) }}"
