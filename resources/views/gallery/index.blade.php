...
@section('title')
    <div class="d-inline-block">
        <h5 class="h5 d-inline-block text-white font-weight-bold mb-0 ">{{ __('Gallery') }}</h5>
    </div>
@endsection
@section('breadcrumb')
...
@section('action-btn')
    <div class="pr-2">
        ...
        @if (Utility::getStorageUsage() >= \Auth::user()->total_storage)
            <a href="#" data-title="{{ __('Create New Gallery') }}" class="disabled btn btn-sm btn-primary btn-icon m-1"
                data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Create') }}">
                <i class="ti ti-plus"></i>
            </a>
        @else
            <a href="#" data-size="md" data-url="{{ route('gallery.create') }}" data-ajax-popup="true"
                data-title="{{ __('Create New Gallery') }}" class="btn btn-sm btn-primary btn-icon m-1"
                            ...
                            <tbody>
                                @foreach ($galleries as $gallery)
                                    <tr>
                                        ...
                                        <td>{{ $gallery->gallery_category() }}</td>
                                        </td>
                                        <td class="Action">
                                            <span>
                                                <div class="action-btn  bg-info ms-2">
                                                    <a href="#" data-size="md"
                                                        data-url="{{ route('gallery.edit', $gallery->id) }}"
