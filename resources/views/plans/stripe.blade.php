...
@php
    ...
    $defaultDuration = 0;
    foreach ($planDurations as $key => $planDuration) {
        $plan->price = $planDuration->price;
        $plan->duration = __(\App\Models\Plan::$arrDuration[$planDuration->duration]);
        $defaultDuration = $planDuration->id;
    
        // $defaultDuration_ = $planDuration->id;
        break;
    }
@endphp
@push('script-page')
    ...
    <script type="text/javascript">
        $(document).on('change', '.plan_duration', function(e) {
            ...
            $('form input[name="plan_duration_id"]').val($(this).val());

            $(".plan-dynamic-info").fadeOut(200, function() {
                $(".plan-dynamic-info").html(price + " / " + duration).fadeIn(200);
            });
        ...
        $(document).ready(function() {
            $(document).on('click', '.apply-coupon', function() {
                ...
                $.ajax({
                    ...
                    data: {
                        plan_id: '{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}',
                        plan_duration_id: '{{ \Illuminate\Support\Facades\Crypt::encrypt($defaultDuration) }}',
                        coupon: coupon
                    },
                    success: function(data) {
                        ...
                        $('#stripe_coupon, #paypal_coupon, #skrill_coupon,#coingate_coupon')
                            .val(coupon);
                        if (data.is_success == true) {
                                        ...
                                        <div class="">
                                            @if (\Auth::user()->type == 'super admin')
                                                <a title="Edit Plan" data-size="lg" href="#" class="action-item"
                                                    ...
                                                    data-bs-placement="top" title="{{ __('Edit Plan') }}"><i
                                                        class="fas fa-edit"></i></a>
                                            @endif
                                        </div>
                                    </div>

                                    <h3 class="mb-4 f-w-600 plan-dynamic-info">
                                        {{ env('CURRENCY_SYMBOL') ? env('CURRENCY_SYMBOL') : '$' }}{{ $plan->price . ' / ' . $plan->duration }}
                                    </h3>
                                    <p>
                                            ...
                                            @foreach ($planDurations as $planDuration)
                                                ...
                                                <div class="form-check form-check-inline mb-3">
                                                    <input type="radio" id="customRadio{{ $i }}"
                                                        name="plan_duration"
                                                        value="{{ \Illuminate\Support\Facades\Crypt::encrypt($planDuration->id) }}"
                                                        class="form-check-input plan_duration"
                                                        data-price="{{ $planDuration->price }}{{ env('CURRENCY_SYMBOL') ? env('CURRENCY_SYMBOL') : '$' }}"
                                                        data-duration="{{ App\Models\Plan::$arrDuration[$planDuration->duration] }}"
                                                        {{ $planDuration->id == $defaultDuration ? 'checked' : '' }}>
                                                    <label class="form-check-label"
                                                        for="customRadio{{ $i }}">{{ App\Models\Plan::$arrDuration[$planDuration->duration] }}
                                                        /
                                                        {{ $planDuration->price }}{{ env('CURRENCY_SYMBOL') ? env('CURRENCY_SYMBOL') : '$' }}</label>
                                                </div>
                                                @php $i++; @endphp
                                            @endforeach
                                        </div>
                                    ...
                                    <ul class="list-unstyled  my-5">
                                        @if ($plan->free_layouts == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('Free Layouts') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Free Layouts') }}
                                            </li>
                                        @endif
                                        ...
                                        @if ($plan->premium_layouts == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('Premium Layouts') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Premium Layouts') }}
                                            </li>
                                        @endif
                                        ...
                                        <li>
                                            <span class="theme-avtar">
                                                <i class="text-primary fas fa-check"></i></span>{{ __('Sub Domain') }}
                                        </li>
                                    @else
                                        <li class="text-danger">
                                            <span class="theme-avtar">
                                                <i class="text-danger fas fa-times-circle"></i></span>{{ __('Sub Domain') }}
                                        </li>
                                    @endif --}}
                                        ...
                                        <li>
                                            <span class="theme-avtar">
                                                <i
                                                    class="text-primary fas fa-check"></i></span>{{ __('Shipping Method') }}
                                        </li>
                                    @else
                                        <li class="text-danger">
                                            <span class="theme-avtar">
                                                <i class="text-danger fas fa-times-circle"></i></span>{{ __('Shipping Method') }}
                                        </li>
                                    @endif --}}
                                        ...
                                        @if ($plan->secure_website_address == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('Secure website address (HTTPS)') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Secure website address (HTTPS)') }}
                                            </li>
                                        @endif

                                        @if ($plan->mobile_optimized == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('Optimized for mobile devices') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Optimized for mobile devices') }}
                                            </li>
                                        @endif

                                        @if ($plan->contact_form == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('Contact Form') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Contact Form') }}
                                            </li>
                                        @endif
                                        ...
                                        @if ($plan->blog == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i class="text-primary fas fa-check"></i></span>{{ __('Blog') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Blog') }}
                                            </li>
                                        @endif
                                        ...
                                        @if ($plan->gallery == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i class="text-primary fas fa-check"></i></span>{{ __('Gallery') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Gallery') }}
                                            </li>
                                        @endif
                                        ...
                                        @if ($plan->job_board == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i class="text-primary fas fa-check"></i></span>{{ __('Job Board') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Job Board') }}
                                            </li>
                                        @endif
                                        ...
                                        @if ($plan->enable_custdomain == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('Custom Domain') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Custom Domain') }}
                                            </li>
                                        @endif
                                        ...
                                        @if ($plan->max_products == '-1')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i class="text-primary fas fa-check"></i></span>{{ __('Unlimited') }}
                                                {{ __('Vehicles') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Unlimited') }}
                                                {{ __('Vehicles') }} ({{ __('Max') }}. {{ $plan->max_products }})
                                            </li>
                                        @endif
                                        ...
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Unlimited') }}
                                                {{ __('Stores') }} ({{ __('Max') }}. {{ $plan->max_stores }})
                                            </li>
                                        @endif
                                        ...
                                        <li>
                                            <span class="theme-avtar">
                                                <i
                                                    class="text-primary fas fa-check"></i></span>{{ __('Disable Product') }}
                                        </li>
                                    @else
                                        <li class="text-danger">
                                            <span class="theme-avtar">
                                                <i class="text-danger fas fa-times-circle"></i></span>{{ __('Disable Product') }}
                                        </li>
                                    @endif --}}

                                        @if ($plan->support_by_mail == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('Email Support') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Email Support') }}
                                            </li>
                                        @endif
                                        @if ($plan->support_by_phone == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('Phone Support') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Phone Support') }}
                                            </li>
                                        @endif
                                        @if ($plan->support_by_whatsapp == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('Whatsapp Support') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Whatsapp Support') }}
                                            </li>
                                        @endif
                                        @if ($plan->api_services == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-primary fas fa-check"></i></span>{{ __('API Services') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('API Services') }}
                                            </li>
                                        @endif
                                        ...
                                        @if ($plan->ad_free == 'on')
                                            <li>
                                                <span class="theme-avtar">
                                                    <i class="text-primary fas fa-check"></i></span>{{ __('Ad-Free') }}
                                            </li>
                                        @else
                                            <li class="text-danger">
                                                <span class="theme-avtar">
                                                    <i
                                                        class="text-danger fas fa-times-circle"></i></span>{{ __('Ad-Free') }}
                                            </li>
                                        @endif
                                    </ul>
                                ...
                                <div class="tab-pane {{ ($admin_payments_details['is_stripe_enabled'] == 'on' && !empty($admin_payments_details['stripe_key']) && !empty($admin_payments_details['stripe_secret'])) == 'on' ? 'active' : '' }}"
                                    ...
                                    <form role="form" action="{{ route('stripe.payment') }}" method="post"
                                        ...
                                        <div class="col-sm-12 my-2 px-2">
                                            <div class="text-end">
                                                <input type="hidden"
                                                    value="{{ \Illuminate\Support\Facades\Crypt::encrypt($defaultDuration) }}"
