...
class CustomerLoginController extends Controller
{
    ...
    public function showLoginForm($slug)
    {
        ...
        return view('storefront.' . $store->theme_dir . '.user.login', compact('blog', 'total_item', 'store', 'slug', 'page_slug_urls','full_page'));
    }
    ...
    public function login(Request $request, $slug, $cart = 0)
    {
        ...
        if ($this->validator($request, $slug) == true) {
            ...
            if (Auth::guard('customers')->attempt(['email' => $email, 'is_verified' => 1, 'password' => $password, 'store_id' => $store_id], $request->filled('remember'))) {

                //Authentication passed...

                //add last login date
                $user = Customer::where('email', $request->email)->where('store_id', $store->id)->first();
    ...
    public function profile($slug, $id)
    {
        ...
        // $page_slug_urls        = PageOption::where('store_id', $store->id)->where('language', App::currentLocale())->get();
