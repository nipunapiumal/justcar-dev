...
Route::get('/test-1', function () {
    ...
    // $requestObject = (object)$request;
    // return new ContactMail($requestObject);
    ...
    //         $datetime2 = new \Datetime("2023-05-19");

    //         $interval = $datetime2->diff($datetime1);
    //         $days = $interval->format('%r%a');

    //         // plan_expire_date
    //         // echo "$days --> " . $user->email . "<br>";
    ...
    // $products = Stor::where('store_id', $store->current_store)->get();

    // foreach ($products as $product) {
    //     $products_images = Product_images::where('product_id', $product->id)->get();
    //     echo "product id " . $product->id . "<br>";
    //     foreach ($products_images as $products_image) {
    //         $image = $products_image->product_images;
    ...
    //             $product_im = Product_images::find($products_image->id);
    //             $product_im["image_size"] = $size;
    //             $product_im->save();
    //         }
    //     }
    //     echo "<br><br>";
    ...
    // $settings = StoreThemeSettings::where('name', 'slider_settings')->get();
    // foreach ($settings as $setting) {
    //     $sliders = json_decode($setting->value);
    //     // print_r($sliders);
    ...
    //         $sliderSettings["slider_image"] = $slider->slider_image;
    //         $sliderSettings["slider_title"] = $slider->slider_title;
    //         $sliderSettings["slider_description"] = $slider->slider_description;
    //         $sliderSettingsFinal[] = $sliderSettings;

    //         $sliderSettingsWithLangs["en"] = $sliderSettingsFinal;
    //         // print_r($sliderSettingsWithLangs);


    //     }

    //     $s =  StoreThemeSettings::find($setting->id);
    //     $s["value"] = json_encode($sliderSettingsWithLangs);
    //     $s->save();
    ...
    // $getStoreThemeSetting = \App\Models\Utility::getStoreThemeSetting(1, 'theme7');
    // $app = App::getLocale();
    // // print_r(json_decode($getStoreThemeSetting["slider_settings"])->$app);
    // $slider_settings = json_decode($getStoreThemeSetting["slider_settings"])->$app;
    // $slider_settings_all = json_decode($getStoreThemeSetting["slider_settings"]);

    // print_r($slider_settings_all);
    ...
    //         $sliderSettings["slider_title"] = $slider_settings_all->$lang[0]->slider_title;
    ...
    //         0 => [
    //             "slider_image" => "header_img_4.png",
    //             "slider_title" => "Title 1 de",
    //             "slider_description" => "Desc 1 de",
    ...
    // echo json_encode($sliderSettings);
    ...
    //     $dir = storage_path("uploads/store_theme/themePlus$i/");
    //     $file1 = $dir . "Home.png";

    //     $fileNameToStore = "Home.jpg";
    //     $img = \Image::make($file1);
    //     $img->save($dir . $fileNameToStore, 30);

    //     echo "$dir . $fileNameToStore saved <br> ";
    // }
    ...
    // App\Models\Utility::defaultEmailUser();
    // App\Models\Utility::defaultEmail2();
    // App\Models\Utility::defaultEmail3();
    // App\Models\Utility::defaultEmail4();
    // App\Models\Utility::defaultEmail5();
    // \App\Models\Utility::defaultEmail6();
    // \App\Models\Utility::defaultEmail7();
    // \App\Models\Utility::defaultEmail8();
    // \App\Models\Utility::defaultEmail9();
    // \App\Models\Utility::defaultEmail10();
    // \App\Models\Utility::defaultEmail11();
    // \App\Models\Utility::defaultEmail12();
    exit;


    // try {

    //     $store = App\Models\Store::where('slug', "primestore")->where('is_store_enabled', '1')->first();
    //     $mail = App\Models\Utility::sendEmailTemplate('New Registration', "dewarajaishan@gmail.com", null, $store, null);
    //     echo print_r($mail);
    // } catch (\Exception $e) {
    //     print_r($e->getMessage());
    // }
    ...
    //     ];
    //     App\Models\ProductLangs::create($data);
...
Route::group(
    ...
    function () {
        ...
        Route::post('owner-manage-language/{slug?}', 'SettingController@manageLanguage')->name('owner.manage.language');
        Route::post('store-sharing-content/{slug?}', 'SettingController@storeSharingContent')->name('store.sharing.content');
        Route::post('mobile-de', 'SettingController@storeMobileDe')->name('store.mobile.de');
        Route::get('layout', 'SettingController@layout')->name('setting.layout');
        Route::get('site-setting', 'SettingController@siteSetting')->name('setting.site_setting');
        Route::get('store-settings', 'SettingController@settings')->name('setting.settings');
        Route::get('payments', 'SettingController@payments')->name('setting.payments');
        Route::get('email-setting', 'SettingController@emailSetting')->name('setting.email_setting');
        Route::get('whatsapp-setting', 'SettingController@whatsappSetting')->name('setting.whatsapp_setting');
        Route::get('twilio-setting', 'SettingController@twilioSetting')->name('setting.twilio_setting');
        Route::get('manage-languages', 'SettingController@manageLanguages')->name('setting.manage_languages');
        Route::get('content-sharing', 'SettingController@contentSharing')->name('setting.content_sharing');

        Route::get('smartsupp-settings', 'SettingController@smartsuppSettings')->name('setting.smartsupp_settings');
        Route::get('cookie-bot-settings', 'SettingController@cookieBotSettings')->name('setting.cookie_bot_settings');

        Route::put('store-live-chat', 'SettingController@storeLiveChat')->name('store.live.chat');
        Route::put('store-cookie-bot', 'SettingController@storeCookieBot')->name('store.cookie.bot');
        // Route::get('store-live-chat-delete', 'SettingController@removeStoreLiveChat')->name('store.live.chat.delete');
        // Route::get('store-cookie-bot-delete', 'SettingController@removeStoreCookieBot')->name('store.cookie.bot.delete');

        Route::get('mobile-de', 'SettingController@mobileDe')->name('setting.mobile_de');
        Route::get('carcopy/test', 'CarCopyController@test')->name('carcopy.test');
        // Route::get('carcopy/search', 'CarCopyController@search')->name('carcopy.search');
        Route::post('carcopy/search', 'CarCopyController@search')->name('carcopy.search');
        Route::get('carcopy/result/{search}', 'CarCopyController@result')->name('carcopy.result');
        Route::get('carcopy/view/{cid}/{search}', 'CarCopyController@view')->name('carcopy.view');
        Route::get('car-hpm', 'CarCopyController@carHPM')->name('carcopy.car_hpm');
        Route::post('car-hpm', 'CarCopyController@storeCarHPM')->name('store.car.hpm');
        Route::get('initial-setup', 'SettingController@initialSetup')->name('setting.initial-setup');
    }
);
...
Route::resource('vehicle_category', 'VehicleCategoryController')->middleware(
    [
        'auth',
        'XSS',
    ]
...
Route::resource('vehicle_tax', 'VehicleTaxController')->middleware(
    [
        'auth',
        'XSS',
    ]
...
Route::get('product/{id}/pdf', 'ProductController@viewPDF')->name('products.view.pdf')->middleware('auth');
Route::get('product/{id}/mobile-de', 'ProductController@mobileDe')->name('product.mobile.de.send')->middleware('auth');
Route::get(
...
Route::group(
    ...
    function () {
        ...
        Route::get('/stripe/{code}/{type?}', 'StripePaymentController@stripe')->name('stripe');
        ...
        Route::post('stripe-pay', 'StripePaymentController@addpaymentV2')->name('stripe.pay');
        Route::post('stripe-pay-v3', 'StripePaymentController@addpaymentV3')->name('stripe.payv3');
        Route::post('stripe-pay-v4', 'StripePaymentController@addpaymentV4')->name('stripe.payv4');
    }
);
...
Route::post('copyright-plan-pay-with-paypal', 'PaypalController@copyrightPlanPayWithPaypal')->name('copyright.plan.pay.with.paypal')->middleware(
    [
        'auth',
        'XSS',
    ]
);
Route::post('storage-plan-pay-with-paypal', 'PaypalController@storagePlanPayWithPaypal')->name('storage.plan.pay.with.paypal')->middleware(
    [
        'auth',
        'XSS',
    ]
);
Route::post('product-plan-pay-with-paypal', 'PaypalController@productPlanPayWithPaypal')->name('product.plan.pay.with.paypal')->middleware(
    [
        'auth',
        'XSS',
    ]
...
Route::post('{slug?}/store-sharing-content/{theme?}', 'StoreController@storeSharingContent')->name('theme.sharing.content')->middleware(['auth']);
...
Route::get('vehicle_request', 'VehicleController@viewVehicleRequests')->name('viewVehicleRequests')->middleware(['auth', 'XSS',]);
...
Route::get('{slug}/user-activate/{remember_token}', 'StoreController@userActivate')->name('store.useractivate');
...
Route::post('/term-and-condition', ['as' => 'term-and-condition.store', 'uses' => 'SettingController@termAndConditionStore'])->middleware(['auth', 'XSS']);
...
// ===================================Vehicles==========================================
Route::get('vehicle/request', 'VehicleController@request')->name('vehicleRequest')->middleware('auth');
Route::post('vehicle/request', 'VehicleController@storeRequest')->name('vehicleRequest')->middleware('auth');
Route::resource('vehicle', 'VehicleController')->middleware(['auth', 'XSS']);
Route::post('product/get-models/{slug?}', 'VehicleController@getModels')->name('product.get-models');
Route::post('product/get-vehicle-brands/{slug?}', 'VehicleController@getVehicleBrands')->name('product.get-vehicle-brands');
Route::resource('vehicle_brand', 'VehicleBrandController')->middleware(['auth', 'XSS']);
Route::resource('vehicle_model', 'VehicleModelController')->middleware(['auth', 'XSS']);
Route::resource('vehicle_type', 'VehicleTypeController')->middleware(['auth', 'XSS']);
Route::post('vehicle/{id}/update', 'VehicleController@update')->name('vehicle.update')->middleware('auth');

Route::resource('fuel-type', 'FuelTypeController')->middleware(['auth', 'XSS']);
Route::resource('vehicle-equipment', 'VehicleEquipmentController')->middleware(['auth', 'XSS']);
Route::resource('exterior-color', 'ExteriorColorController')->middleware(['auth', 'XSS']);
Route::resource('interior-color', 'InteriorColorController')->middleware(['auth', 'XSS']);
Route::resource('interior-design', 'InteriorDesignController')->middleware(['auth', 'XSS']);


// ===================================Advertisements==========================================
Route::resource('advertisements', 'AdvertisementController')->middleware(['auth', 'XSS']);

// ===================================Contact==========================================
Route::get('{slug?}/contact', 'StoreController@contact')->name('store.contact');
Route::post('{slug?}/contact', 'StoreController@storeContact')->name('store.store-contact');


// ===================================Gallery==========================================
Route::resource('gallery', 'GalleryController')->middleware(['auth', 'XSS']);

// ===================================Gallery Categorie==========================================
Route::resource('gallery_categorie', 'GalleryCategorieController')->middleware(['auth', 'XSS']);

// ===================================Gallery (Store Area)==========================================
Route::get('{slug?}/gallery/{category?}', 'StoreController@gallery')->name('store.gallery');

// ===================================Job Board==========================================
Route::resource('job-board', 'JobBoardController')->middleware(['auth', 'XSS']);
Route::get('{slug?}/job-board/apply/{job_board}', 'JobApplicantController@create')->name('store.job-board.create');

// ===================================Job Board (Store Area)==========================================
Route::get('{slug?}/job-board', 'JobBoardController@show')->name('store.job-board');

// ===================================Job Categorie==========================================
Route::resource('job_categorie', 'JobCategorieController')->middleware(['auth', 'XSS']);

// ===================================Job Applicants==========================================
Route::resource('job-applicants', 'JobApplicantController')->middleware(['auth', 'XSS']);


Route::get('/privacy/{lang?}', 'DashboardController@privacy')->name('privacy');
Route::get('/terms/{lang?}', 'DashboardController@terms')->name('terms');

Route::resource('plan_duration', 'PlanDurationController')->middleware(['auth', 'XSS']);

Route::resource('storage_plans', 'StoragePlanController')->middleware(['auth', 'XSS']);
Route::resource('copyright-plan', 'CopyrightPlanController')->middleware(['auth', 'XSS']);
Route::resource('product-plan', 'ProductPlanController')->middleware(['auth', 'XSS']);
Route::resource('carcopy', 'CarCopyController')->middleware(['auth', 'XSS']);
Route::resource('auto-gallery', 'AutoGalleryController')->middleware(['auth', 'XSS']);

Route::get('update-free-trial-msg', 'SettingController@updateFreeTrialMsg')->name('update-free-trial-msg');

Route::resource('users', 'UserController')->middleware(['auth', 'XSS']);
Route::resource('localization', 'LocalizationController')->middleware(['auth', 'XSS']);
Route::resource('invoice', 'InvoiceController')->middleware(['auth', 'XSS']);
Route::get('user/lang-specialists', 'UserController@getLangSpecialists')->name('users.lang-specialists')->middleware(['auth']);
Route::get('user/edit-lang-specialist/{id}', 'UserController@editLangSpecialist')->name('users.edit-lang-specialist')->middleware(['auth']);
Route::post('user/edit-lang-specialist/{id}', 'UserController@updateLangSpecialist')->name('users.edit-lang-specialist')->middleware(['auth']);
Route::get('user/accept-terms', 'UserController@acceptTerms')->name('users.accept-terms')->middleware(['auth']);


// ===================================Forum==========================================
Route::get('forum/threads', 'ThreadController@viewThreads')->middleware(['auth'])->name('forum-threads');
Route::get('forum/discussions/{name?}', 'ThreadController@search')->name('forum.search');
Route::resource('forum', 'ThreadController')->middleware(['auth', 'XSS']);
Route::post('forum/{id}/update', 'ThreadController@update')->name('forum.update')->middleware('auth');
Route::resource('forumzone', 'ThreadCategoryController')->middleware(['auth', 'XSS']);
Route::post('forumzone/{id}/update', 'ThreadCategoryController@update')->name('forumzone.update')->middleware('auth');

Route::resource('thread_reply', 'ThreadReplyController')->middleware(['auth', 'XSS']);